...
подтвердил регион, **напечатал запрос в поиск** и отправил его, прошёл по карточкам
вакансий, нашёл блок требований через `browser_find`, сложил данные по каждой вакансии
в `note` — и выдал три реальные вакансии с компаниями, зарплатами и требованиями плюс
сопроводительное письмо под выбранную.

```
30 steps · 36 model calls · 132 563 in / 9 991 out · cache read 266 428
compactions 0 · pruned observations 21 · status: completed
```
...
* **`back` в новой вкладке ничего не делает.** hh.ru открывает вакансии в новых
  вкладках, `go_back()` возвращает `None`, страница не меняется — и агент бьётся об
  это раз за разом. Теперь действие честно говорит, что истории нет, и указывает на
  `browser_tabs`.

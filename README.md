...
```bash
.venv/bin/python -m pytest -q      # 30 тестов, реальный браузер, без обращений к API
```
...
```bash
.venv/bin/browser-agent --demo --record-video --slow-mo 120 "…задача…"
.venv/bin/python scripts/make_video.py runs/<ts> -o demo.mp4
```
